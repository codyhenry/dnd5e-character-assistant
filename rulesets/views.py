...
class RulesetCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form: RulesetForm) -> HttpResponse:
...
@login_required
@require_POST
def add_option_restriction_view(request: HttpRequest, pk: int) -> HttpResponse:
    ...
    detail_view.setup(request, pk=pk)
...
@login_required
@require_POST
def remove_option_restriction_view(
    request: HttpRequest,
    pk: int,
    restriction_pk: int,
) -> HttpResponse:
